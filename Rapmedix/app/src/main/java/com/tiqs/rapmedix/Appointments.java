...
import com.tiqs.rapmedix.adapters.Appointments_adapter;

public class Appointments extends AppCompatActivity {
    String [] tabs={"Active","Pending","Completed","Cancelled"};
    TabLayout tabLayout;
    ViewPager viewPager_home;
    Appointments_adapter add_member_adapter;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        add_member_adapter=new Appointments_adapter(getSupportFragmentManager());
        ...
        if (getIntent()!=null)
        {
            if (getIntent().getIntExtra("Pending_Intent",0)==1)
            {
                tabLayout.getTabAt(1).select();
            }
            if (getIntent().getIntExtra("Pending_Intent",0)==2)
            {
